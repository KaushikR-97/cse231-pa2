...
function codeGenStmt(stmt, env) {
    switch (stmt.tag) {
        ...
        case "if":
            ...
            var thnStmts = stmt.thn.map(function (innerStmt) { return codeGenStmt(innerStmt, env); }).flat();
            var elsStmts = stmt.els.map(function (innerStmt) { return codeGenStmt(innerStmt, env); }).flat();
            return condExpr
                .concat(["(if (then"])
                .concat(thnStmts)
                .concat([")", "(else"])
...
function codeGenInit(init, env) {
    ...
    if (env.locals.has(init.name)) {
        ...
        // `(i32.const ${envLookup(env, init.name)})`,
...
function tcStmt(env, locals, stmt) {
    switch (stmt.tag) {
        ...
        case "if":
            ...
            var elsTyp = tEls[tEls.length - 1].a;
            // restore loop depth
            locals.loop_depth -= 1;
            console.log(tCond.a);
